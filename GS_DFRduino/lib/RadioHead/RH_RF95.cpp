...
bool RH_RF95::init()
{
    // Serial.println("RF95 init start");
    
    if (!RHSPIDriver::init()) {
        // Serial.println("RF95: RHSPIDriver init failed");
        return false;
    }
    // Serial.println("RF95: SPI driver init OK");

#ifdef RH_USE_MUTEX
    if (RH_MUTEX_INIT(lock) != 0) { 
        Serial.println("RF95: mutex init failed");
        return false;
    }
    Serial.println("RF95: mutex init OK");
#endif

    if (!setupInterruptHandler()) {
        // Serial.println("RF95: interrupt handler setup failed");
        return false;
    }
    // Serial.println("RF95: interrupt handler setup OK");

    // Set sleep mode, so we can also set LORA mode:
    spiWrite(RH_RF95_REG_01_OP_MODE, RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE);
    delay(10); // Wait for sleep mode to take over from say, CAD
    // Serial.println("RF95: sleep mode set, waiting to take effect");
    
    // Check we are in sleep mode, with LORA set
    uint8_t mode = spiRead(RH_RF95_REG_01_OP_MODE);
    // Serial.print("RF95: read mode register: 0x");
    Serial.println(mode, HEX);
    
    if (mode != (RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE))
    {
        // Serial.println("RF95: failed to enter sleep mode with LoRa set");
        // Serial.print("RF95: expected 0x");
        Serial.print(RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE, HEX);
        // Serial.print(" but got 0x");
        Serial.println(mode, HEX);
        return false; // No device present?
    }
    // Serial.println("RF95: successfully entered sleep mode with LoRa set");

    // Rest of the initialization code...
    // ...
    
    // Serial.println("RF95: init complete success");
    return true;
}
