...
try:
    zwoasi.init('ZWO_ASI_LIB/lib/x64/ASICamera2.dll')  # Replace with your actual path
    print("✅ ASICamera2.dll loaded successfully.")
except Exception as e:
    print("❌ Failed to load SDK:", e)
...
# if args.filename:
#     zwoasi.init(args.filename)
# elif env_filename:
#     zwoasi.init(env_filename)
# else:
#     print('The filename of the SDK library is required (or set ZWO_zwoasi_LIB environment variable with the filename)')
#     sys.exit(1)
...
cameras_found = zwoasi.list_cameras()  # Models names of the connected cameras
...
camera.set_control_value(zwoasi.ASI_EXPOSURE, 1000)
...
for c in controls:
    if controls[c]['ControlType'] == zwoasi.ASI_BANDWIDTHOVERLOAD:
        continue
...
timeout = (camera.get_control_value(zwoasi.ASI_EXPOSURE)[0] / 1000) * 2 + 500
